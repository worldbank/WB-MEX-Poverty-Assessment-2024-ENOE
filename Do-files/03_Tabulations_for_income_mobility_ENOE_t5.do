		...
		*use `vars' `ifuse' using "$path/PANEL/DATA/MEX_2005_2023_PANEL_QUARTER_EXPORTS.dta",clear
		...
		preserve
			...
			if `set'==7 {
				...
				collapse (mean) slider stayer climber neverpoor if n_ent==1 [fw=tamh], by(year quarter `myvar')
				drop year quarter `myvar'
			}
			
			if "`monetaryunit'"=="lcu" export excel using "$output/paneles para estudio movilidad de ingresos.xlsx", sheetmodify sheet("`outputsheet'") cell(`op'`oc')
			if "`monetaryunit'"=="ppp" export excel using "$output/paneles para estudio movilidad de ingresos.xlsx", sheetmodify sheet("`outputsheet'ppp") cell(`op'`oc')
