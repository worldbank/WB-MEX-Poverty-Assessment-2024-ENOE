...
<_Program name_>				[MEX_2009_ENOE_V01_M_V06_A_GLD_ALL.do] </_Program name_>
...
<_Data collection to_>			[05/2009] </_Data collection to_>
<_Source of dataset_> 			[Mexico NSO] </_Source of dataset_>
<_Sample size (HH)_> 			[105129] </_Sample size (HH)_>
<_Sample size (IND)_> 			[394435] </_Sample size (IND)_>
...
* Date: [2022-03-D7] - [corrections variable wmonths, use of correct version for isic]
...
local path_in_stata "`server'/`country' copy/`level_1'-`quarter'/`level_2_mast'/Data/Stata"
local path_in_other "`server'/`country' copy/`level_1'-`quarter'/`level_2_mast'/Data/Original"
local path_output   "`server'/`country' copy/`level_1'-`quarter'/`level_2_harm'/Data/Harmonized"
...
***then first job
	tostring p3, gen(cmo)
	merge m:1 cmo using "`path_in_stata'/CMO_09_ISCO_08.dta", keep(master match) nogen
*Note: rename necessary to allow for the second job code to generate a new cmo for the merge
	rename cmo cmo_1
	rename isco isco_1
...
*<_hsize_>
*the missing values created as a result correspond to individuals classified as house workers or other non related relatives that inhabit a household but have not been identified as part of the household. (codes from 400 to 599 and from 700 to 999 )
...
*<_wmonths_>
	...
	replace wmonths=. if p5g13==13 | p5g15==15 | p5g99==99
...
replace firmsize_l = 101 if p3q==09
replace firmsize_l = 251 if p3q==10
replace firmsize_l = 501 if p3q==11
...
replace firmsize_u=100 if p3q==08
replace firmsize_u = 250 if p3q==09
replace firmsize_u = 500 if p3q==10
replace firmsize_u = . if p3q==11
...
label var firmsize_u "Firm size (lower bracket) primary job 7 day recall"
